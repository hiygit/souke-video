      ...
      <van-collapse v-model="activeNames">
        <van-collapse-item title="课程讲师" name="1">
          ...
          >
            ...
            <div
              class="courseLecturerList-text"
              :class="items.ismore ? 'heightauto' : ''"
              v-html="items.teacherSynopsis"
            ></div>
            <div
              class="flexible teachmore"
              style="height: 1.9rem;line-height: 2.5rem;"
              @click="showmore(items)"
            >
              {{ items.ismore ? "收起" : "更多" }}
              <i :class="items.ismore ? 'up' : 'down'"></i>
            </div>
          </div>
        </van-collapse-item>
      </van-collapse>
    ...
    <div class="courseOutline" v-if="chapterlist.length == 1">
      <div class="courseTitle">
        ...
        <div class="authorizationTab">
          <span> <i class="dot"></i> 已授权 </span>
          <span> <i class="dot" style="background: #BEBEBE"></i> 未授权 </span>
        </div>
      </div>
      <!-- 课程大纲 课时 -->
      <div class="courseOutlineListBox">
        <van-collapse
          ...
          accordion
          v-for="(item, index) in chapterlist[0].courseHourList"
          :key="'item' + index"
        >
          <van-collapse-item :name="index" class="moveDownIcon">
            ...
            <span
              class="toLook"
              :class="item.isLock == '0' ? '' : 'notolook'"
              @click="gotolook(item)"
              >去观看</span
            >
            <div
              class="courserExerciseBox"
              @click="
                topractice(
                  item.id,
                  item.teacherIssue ? item.teacherIssue.id : ''
                )
              "
              v-show="item.hasBank == '1'"
            >
              <div class="courserExerciseBoxLeft">
                <span class="titleColor">课程练习</span>
                <p class="hour-time" v-show="item.teacherIssue.createDate">
                  {{ item.teacherIssue.createDate | formatDate }}更新
                </p>
              </div>
              <span
                class="toLook"
                :class="item.hasBank == '1' ? '' : 'notolook'"
                >去答题</span
              >
            </div>
            <div class="studymsg" v-show="item.meansList">
              ...
              <div
                ...
                v-for="(item1, index1) in item.meansList"
                :key="'item1' + index1"
              >
                <p>{{ item1.meansName }}</p>
                <span class="toLook" @click="checkfile(item1, item.id)"
                  >查看</span
                >
              </div>
            </div>
    ...
    <div class="courseOutline" v-else>
      ...
      <div class="courseSection">
        <van-collapse
          ...
          accordion
          v-for="(items, index) in chapterlist"
          :key="'c' + index"
        >
          <van-collapse-item :title="items.chapterName" :name="index">
            ...
            <p
              class="textHigh"
              :class="items.ismore ? 'heightauto' : ''"
              v-html="items.chapterSynopsis"
            ></p>
            <div class="flexible" @click="showmore(items)">
              {{ items.ismore ? "收起" : "更多" }}
              <i :class="items.ismore ? 'up' : 'down'"></i>
            </div>
          </van-collapse-item>
      ...
      <div class="courseOutline">
        ...
        <div class="courseOutlineListBox">
          <van-collapse
            v-model="activeNames1"
            accordion
            v-for="(item, index) in currentCourseHourList"
            :key="'item' + index"
          >
            <van-collapse-item :name="index" class="moveDownIcon">
              <div slot="title">
              ...
              >
                <div class="courserExerciseBoxLeft">
                  <span class="titleColor">课程练习</span>
                  <!-- 这里的更新时间有问题 -->
                  <p class="hour-time" v-show="item.createDate">
                    {{ item.createDate | formatDate }}更新
                  </p>
                  <!-- 这里的更新时间有问题 -->
                </div>
                <span
              ...
              <div class="studymsg" v-show="item.meansList">
                ...
                <div
                  class="courserExerciseBox"
                  style="border-bottom: none;display:flex;align-items:center;"
                  v-for="(item1, index1) in item.meansList"
                  :key="'item1' + index1"
                >
                  <p>{{ item1.meansName }}</p>
                  <div style="display:flex;justify-content:space-between;">
                    <span class="toLook" style="margin:0 5px 0 0;" @click="checkfile(item1, item.id)"
                      >查看</span
                    >
                    <span class="toLook" style="margin:0 5px 0 0;" @click="download(item1.fileUrl)"
                      >下载</span
                    >
                  </div>
                </div>
...
export default {
  ...
  methods: {
    ...
    getCourseDetails() {
      ...
      this.$ajax
        .post(
          this.GLOBAL.BASE_URL + "/courses/findCourseInfo",
          qs.stringify({ userId: userId, courseId: that.courseId, fromType: 1 })
        )
        .then(res => {
          ...
          if (res.data.success && res.data.errorCode == 2) {
            ...
            this.courseDetails.hotImage =
              baseUrl + res.data.body.course.hotImage;
    ...
    topractice(hourid, id) {
      this.$router.push({
        path: "/lineexam",
        query: { hourId: hourid, issueId: id, fromType: 1 }
      });
    },
    //去观看课程视频
    gotolook(item) {
    ...
    checkfile(item, hourId) {
      ...
      this.$ajax
        .post(
          this.GLOBAL.BASE_URL + "/courses/updateStatus",
          qs.stringify({
            hourId: hourId,
        ...
        .then(res => {
          ...
          if (res.data.success && res.data.errorCode == 2) {
            ...
            this.$ajax
              .post(
                this.GLOBAL.BASE_URL + "/courses/updateViewCount",
